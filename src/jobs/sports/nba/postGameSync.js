/**
 * nba.postGameSync.js — NBA-only game lifecycle + outcome grading
 *
 * Handles:
 ...
 *   node -e "require('./nba.postGameSync').run().then(r => console.log(JSON.stringify(r,null,2)))"
 */
...
const SPORT = 'nba';
const FINALIZE_AFTER_HOURS = Number(process.env.NBA_FINALIZE_AFTER_HOURS || process.env.POST_GAME_FINALIZE_AFTER_HOURS || 3.5);
const STALE_DELETE_AFTER_HOURS = Number(process.env.NBA_STALE_DELETE_AFTER_HOURS || process.env.POST_GAME_STALE_DELETE_AFTER_HOURS || 30);
const OUTCOME_MAX_RETRY_ATTEMPTS = Math.max(1, parseInt(process.env.OUTCOME_MAX_RETRY_ATTEMPTS || '12', 10));

async function run() {
  logger.info(`🔄 [NBAPostGameSync] Starting...`);
  ...
  if (toFinal.length) {
    ...
    const finalEventIds = toFinal.map(g => g.oddsEventId).filter(Boolean);
      const outcomeResult = await gradeEvents(finalEventIds, { sport: SPORT, source: 'nba.postGameSync.finalize' });
  ...
  if (finalGames.length) {
    ...
    if (unresolvedCount > 0) {
        const reGrade = await gradeEvents(ids, { sport: SPORT, source: 'nba.postGameSync.regrade' });
  ...
  if (stale.length) {
    const staleEventIds = stale.map(g => g.oddsEventId).filter(Boolean);
      ...
      await gradeEvents(staleEventIds, { sport: SPORT, source: 'nba.postGameSync.stale' });
  ...
  logger.info(`✅ [NBAPostGameSync] Done`, {
    changes,
    deleted,
