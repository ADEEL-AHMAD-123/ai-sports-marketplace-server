...
class InsightService {
  ...
  async generateInsight({
    ...
    if (existing && existing.status !== 'stale') {
      ...
      // even when the insight was originally generated by someone else. The
      // cache save is on us (skip OpenAI + re-processing), not on the user's
      // credit balance. Only same-user re-unlocks stay free — that's handled
      // by the controller's early-return path and by the check below.
      //
      // System-generated cache hits (outcome coverage job — user=null) never
      // deduct or track unlocks; they exist only for grading + proof.
      let cacheHitCreditDeducted = false;
      if (user && !user.hasUnlockedInsight(existing._id)) {
        if (!user.hasEnoughCredits(CREDITS.COST_PER_INSIGHT)) {
          logger.warn('💸 [InsightService] Cache hit but user has insufficient credits', {
            ...logCtx, credits: user.credits,
          });
          return {
            insight: null,
        ...
        // Deduct credit + mark unlocked + write a ledger row (same code path
        // as a freshly-generated insight, so history looks identical).
        // _deductCredit also mutates the in-memory user so the controller's
        // remainingCredits calc is accurate without a re-fetch.
        await this._deductCredit({ user, insight: existing, logCtx });
        await Insight.findByIdAndUpdate(existing._id, { $inc: { unlockCount: 1 } });
        cacheHitCreditDeducted = true;
      }
  ...
  async _deductCredit({ user, insight, logCtx }) {
    ...
    // Also update the in-memory user object so callers can trust
    // `user.credits` and `user.unlockedInsights` without a re-fetch.
    user.credits = newBalance;
    if (!user.unlockedInsights) user.unlockedInsights = [];
    if (!user.unlockedInsights.some((id) => id.toString() === insight._id.toString())) {
      user.unlockedInsights.push(insight._id);
    }
